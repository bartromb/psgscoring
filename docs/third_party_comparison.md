...
| 2026-08-14 | lange events splitsen op fysiologische ankers | CAISR-resp (`split_long_events`) | > 60 s, splits op arousals of desaturaties | `split_events_longer_than_s`, default `None`; desaturatie-onsets eerst, arousals als terugval, ankers binnen 10 s van een rand genegeerd | SN3 (dichte events) en MESA-holdout | *geïmplementeerd, nog niet gemeten* |
| 2026-08-14 | 2 %-desaturatie onder een lage baseline | CAISR-resp (`SpO2_drop_analysis`) | binair: 2 % telt als 3 % onder de 2-min-baseline | `desat_low_baseline_relaxation`, default `False`; **gegradeerd** — het sigmoidcentrum schuift 3 % → 2 %, breedte gelijk | hoe vaak de conditie vuurt; daarna kliniekcasussen en MESA-subgroep met lage baseline | *geïmplementeerd, nog niet gemeten — **regelafwijking**, nooit default aan* |
